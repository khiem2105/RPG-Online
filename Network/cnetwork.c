...
struct MasterPeer {
    ...
    int max_peer;
    // Keep track of number of player
    int number_of_peers;
...
struct NormalPeer {
    ...
    struct PeerSocket peer_socket[5];
    // table of other peer's who connected before THIS PEER
    struct PeerSocket old_peer_socket[5];
    int number_of_old_peers;
    int max_peer;
    int myId;
    // variable which maintains the master loop 
    bool running_master_thread;
...
void Cinit_normal_peer(void) {
    ...
    Peer.number_of_old_peers = 0;
    memset(Peer.peer_socket, 0, sizeof(Peer.peer_socket));
    memset(Peer.old_peer_socket, 0, sizeof(Peer.old_peer_socket));
}
//--------------------END PEER ----------------------
...
void Clisten_and_accept(void) {
    if (DEBUG) printf("[C] Listener on port %d \n", Master.port);
...
void Cpeer_create_bind_listen_and_accept(void) {
	...
	stop("[C] Bind() failed : ");
    if (DEBUG) printf("[C] Peer : Create and bind port %i successfully!\n", Peer.port);
    // listen 
    if (DEBUG) printf("[C] Peer listen on port %i \n", Peer.port);
...
void Ccreate_and_bind(void) {
	...
	stop("[C] Bind() failed : ");
    if (DEBUG) printf("[C] Create and bind successfully!\n");
}
...
void Cincomming_connection(void) {
	...
	if (DEBUG) printf("[C] New connection , socket fd is %d , ip is : %s , port : %d \n" , new_socket , new_connection_ip, new_connection_port);

	Master.number_of_peers += 1;
...
void Cincomming_message(void) {
	...
	if (FD_ISSET(sd, &Master.readfds)) {
		...
		sprintf(master_buffer, "Disconnected;%i;", i);
	    }
...
void* Cmaster_loop(void *args) {
	...
	/*if (DEBUG) printf("[C] Thread C is running \n");*/
    ...
    if (DEBUG) printf("[C] Master thread stopped!\n");
    return NULL;
}
...
void Cmaster_peer_start_loop(void) {
    ...
    // Create Thread LOOP
    if (DEBUG) printf("[C] Python create C-master-thread - multiplayers!\n");
...
static PyObject* peer_receive_from_old_peer(PyObject* self, PyObject* args) {
    ...
    return Py_BuildValue("s", Cpeer_receive_from_old_peer(id));
}
// ---------------------------------------------------------
// ---------------------------------------------------------
...
int Cpeer_connect_to(int connect_port, char* connect_ip) {
	...
	if (DEBUG) printf("[C] Connected to %s\n" , connect_ip);
    return socket_desc;
}
...
static PyObject* peer_connect_to_peer(PyObject* self, PyObject* args) {
    ...
    if (DEBUG) printf("[C] Connecting to the peer whose port : %i and ip: %s ... " ,connect_port, connect_ip);
    // Add file descriptor of new peer to a table
    if (Peer.number_of_other_peers < MAX_CONNECTION) {
	int p = Peer.number_of_old_peers; 
	Peer.number_of_old_peers++;
	Peer.old_peer_socket[p].fd =  Cpeer_connect_to(connect_port, connect_ip);
	Peer.old_peer_socket[p].ip = connect_ip;
	Peer.old_peer_socket[p].port = connect_port;
	Peer.old_peer_socket[p].id = connect_id;
	if (DEBUG) printf("[C] Connected to player %i! Added to old_peer_socket\n", connect_id);
	// Send id to the peer connected
	// First message + id : F%i
	sprintf(global_buf, "F%i", Peer.myId);
	Csend_message(Peer.old_peer_socket[p].fd , global_buf);
	/*int sd = Peer.old_peer_socket[p].fd;*/
	// if sd==0 : haven't  connection
	// else : it's a valid socket descriptor => add to the list
	...
	if (fcntl(Peer.old_peer_socket[p].fd, F_GETFL) & O_NONBLOCK) 
	    if (DEBUG) printf("Socket is non-blocking!\n");
	if (fcntl(Peer.old_peer_socket[p].fd, F_SETFL, fcntl(Peer.old_peer_socket[p].fd, F_GETFL) | O_NONBLOCK) < 0)
	    stop("Fcntl error in peer_connect_to_peer : \n");
	
    } else {
	if (DEBUG) printf("[C] Can't establish new connection because it reached the maximum !\n");
    }
    return Py_BuildValue("s", "Success");
}
...
static PyObject* peer_connect_to_master(PyObject* self, PyObject* args) {
    ...
    if (DEBUG) printf("[C] Connecting to master port : %i, ip: %s ... " ,connect_port, connect_ip);
    Cinit_normal_peer();
...
void Cpeer_incomming_connection(void) {
	...
	if (DEBUG) printf("[C] Connected socket fd is %d , ip is : %s , port : %d \n" , new_socket , new_connection_ip, new_connection_port);
	...
	for (int i=0; i< Peer.max_peer; i++) {
		...
		if (DEBUG) printf("Add to Peer.peer_socket [i] with i = %d\n", i);
		break;
	    } 
	}
    }
...
void Cpeer_incomming_message(void) {
	...
	if (FD_ISSET(sd, &Peer.readfds)) {
		...
		if (Peer.buffer[0] == 'F') {
		    ...
		    if (DEBUG) printf("[C] Connected to player %i\n", int_c);
...
void* Cpeer_loop(void *args) {
    if (DEBUG) printf("[C] Peer's loop is running ...\n");
    // while running is True : 
    // 1. listen and accept others Peers 
    // 2. when accept => send welcome packet 
    // (all the addresse of others peers)
    ...
    if (DEBUG) printf("[C] Peer thread stopped!\n");
    return NULL;
}
...
void Cnormal_peer_start_loop(void) {
    ...
    // Create Thread LOOP
    if (DEBUG) printf("[C] Python create C thread : run peer_loop!\n");
...
static PyObject* master_get_message_from_player(PyObject* self, PyObject* args) {
    int id ;
    if (!PyArg_ParseTuple(args, "i", &id)) return NULL;
    // This check is not nesscessary but I do it anyway beause of semantic
    for (int i=0; i < Master.number_of_peers; i++)  {
	if (id == Master.peer_socket[i].id) {
	    if (Master.peer_socket[i].last_message[0] != '\0') {
		char buffer2[BUF_SIZE]; 
		strcpy(buffer2, Master.peer_socket[i].last_message);
		bzero(Master.peer_socket[i].last_message, BUF_SIZE);
		return Py_BuildValue("s",buffer2); 
	    }
	    break;
	} 
...
// -------------------------------------------------------
// -------------------- Close Socket -------------------------
static PyObject* close_socket(PyObject* self) {
    if (Master.master_socket) close(Master.master_socket);
    if (Peer.master_socket) close(Peer.master_socket);
    return Py_BuildValue("s", "Success");
}
// ---------------------------------------------------------
// ---------------------------------------------------------
...
// Peer send message to another peer with specific ID
void Cpeer_send_to_peer_with_id(char* message, int id) {
	for(int i = 0; i < Peer.max_peer; i++) {
		if(Peer.peer_socket[i].fd != 0 && Peer.peer_socket[i].id == id)
			Csend_message(Peer.peer_socket[i].fd, message);
	}
}

static PyObject* peer_send_to_peer_with_id(PyObject* self, PyObject* args) {
	char* message, id;
	if(!PyArg_ParseTuple(args, "si", &message, &id))
		return NULL;
	printf("Send %s to player with id %d", message, id);
	Cpeer_send_to_peer_with_id(message, id);
	return Py_BuildValue("s", "Success");
}
...
static PyMethodDef networkMethods[] = { // (PyCFunction)
	...
	{"peer_send_to_peer_with_id", peer_send_to_peer_with_id, METH_VARARGS, "Player send message to an another player by using id"},
    ...
    {"close_socket", (PyCFunction)close_socket, METH_NOARGS, "Closing all socket"},
